#include "PhoenixGyro.h"

namespace PhoenixLib
{
	PhoenixGyro::PhoenixGyro(uint32_t channel)
		:
		m_RingBuffer(rngCreate(sizeof(st_Accumulation) * PhoenixGYRO_RING_SIZE)),
	...
	PhoenixGyro::~PhoenixGyro()
	{
		delete m_Analog;
		rngDelete(m_RingBuffer);
	}
	
	double PhoenixGyro::TimeSinceLastSave()
	{
		return Timer::GetFPGATimestamp() - m_LastSaveTime;
	}
	
	double PhoenixGyro::GetAngle()
	{
	...
	void PhoenixGyro::Reset()
	{
		m_Analog->ResetAccumulator();
	}
	
	void PhoenixGyro::HandleCalibration()
	{
		if(TimeSinceLastSave() > PhoenixGYRO_ACCUMULATION_PERIOD)
		{
			st_Accumulation acc;
			m_Analog->GetAccumulatorOutput(&acc.value, &acc.count);
			
			if(m_RecalQueued && rngNBytes(m_RingBuffer) == sizeof(st_Accumulation)*(PhoenixGYRO_RING_SIZE - 1))
			{
	...
	void PhoenixGyro::FinalizeCalibration()
	{
		if((unsigned int)rngNBytes(m_RingBuffer) < sizeof(st_Accumulation) * PhoenixGYRO_RING_SIZE)
		{
		...
		st_Accumulation acc[PhoenixGYRO_RING_SIZE] = {0};
		rngBufGet(m_RingBuffer, (char*)&acc, sizeof(st_Accumulation) * PhoenixGYRO_RING_SIZE);
		for(int i = 0; i < PhoenixGYRO_RING_SIZE; i++)
		{
		...
		printf("Done with PhoenixGyro calibration!\nValue:\t%lld\tCount:%d\nCenter:\t%lld\tOffset:\t%f\n", totalValue, totalCount, m_Center, m_Offset);
	...
	void PhoenixGyro::SetSensitivity(double voltsPerDegreePersecond)
	{
